// Package cmd holds the command line surface of the collector: flag parsing,
// logging setup and the wiring between collection and reporting.
package cmd
...
import (
	...
	"github.com/licenseware/ojdm-collector/internal/logging"
	"github.com/licenseware/ojdm-collector/internal/report"
)

// Run parses the command line and performs a collection run.
func Run(build Build) {
	...
	if *showVersion {
		fmt.Println(versionString(build))
		return
	}
	...
	records := javainfo.Collect(trimSpaths, log)

	meta := report.Meta{CollectedAt: runStartedAt, Version: build.Version}

	if err := report.WriteCSV(*csvReportPath, records, meta, log); err != nil {
